 ...
 *      G04_File[] listFiles(FileFilter filter) 返回抽象路径名数组
 *          public interface FileFilter 用于抽象路径名的过滤器。
 *              作用：用来过滤文件（File对象）
 *              抽象方法：用来过滤文件的方法
 *                  boolean accept(G04_File pathname) 测试指定抽象路径名是否应该包含在某个路径名列表中。
 *                      参数：pathname - 要测试的抽象路径名
 *      G04_File[] listFiles(FilenameFilter filter) 返回抽象路径名数组
 *          public interface FilenameFilter 实现此接口的类实例可用于过滤器文件名。
 *              作用：用于过滤文件名称
 *              抽象方法：用来过滤文件的方法
 *                   boolean accept(G04_File dir, String name) 测试指定文件是否应该包含在某一文件列表中。
 *                   参数：
 *                      dir - 构造方法中传递的被遍历的目录
...
public class Demo06FileFilter {
    public static void main(String[] args) {
        //getAllFile(new G04_File("D:\\personalDoc\\2019\\August"));
        getAllFile2(new File("D:\\personalDoc\\2019\\August"));
    }
    public static void getAllFile(File file){
        //G04_File[] files = file.listFiles(new FileFilterImpl());
        //可以使用匿名内部类来实现这个接口,同时该接口只有一个抽象方法，所以可以使用lambda进行优化
        /*G04_File[] files = file.listFiles(new FileFilter() {
            @Override
            public boolean accept(G04_File pathname) {
    ...
    public static void getAllFile2(File file){
        /*G04_File[] files = file.listFiles(new FilenameFilter() {
            @Override
            public boolean accept(G04_File dir, String name) {
